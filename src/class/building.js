...
export default class Building {
		...
		loader.load('/model/main.fbx', function (object) {
			object.traverse(function (child) {
				if (child.isMesh) {
					...
					let mesh = new THREE.LineSegments(child.geometry, lineMaterial);
					mesh.rotation.x = Math.PI / 2;
					that.objGroup.add(mesh);
				}
			});
			that.objGroup.add(object);
			// let helper = new THREE.BoundingBoxHelper(that.objGroup, 0xff0000);
			// helper.update();
	...
	composer() {
		...
		let renderPersonScene = new RenderPass(this.scene, this.camera);
		...
		this.sceneComposer.addPass(renderPersonScene);
